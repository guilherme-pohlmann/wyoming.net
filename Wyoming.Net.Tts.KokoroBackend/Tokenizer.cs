...
public static partial class Tokenizer
{
    ...
    {
        var strings = await Task.WhenAll(Tokenizer.PhonemeLiteral().Split(inputText).Select(async text =>
        {
            var match = Tokenizer.PhonemeLiteral2().Match(text);
    ...
    private static string  PreprocessText(string text, string langCode = "en-us")
    {
        ...
        text = Tokenizer.HeaderImgLink().Replace(text, "$1$2");
        text = Tokenizer.HeaderLink().Replace(text, "$1");

        text = Tokenizer.CodeBlock().Replace(text, ProcessCodeBlock);
        text = Tokenizer.TickQuote().Replace(text, m => ProcessInlineCode(m.Groups[1].Value));

        var rx = Tokenizer.Money();
